...
const WorkOrderList = ({ user, selectedOrder, setSelectedOrder }) => {
  ...
  const { workOrders, setWorkOrders, loading, error, fetchWorkOrders } = useFetchWorkOrders(token); // Get setWorkOrders from the hook
  ...
  useEffect(() => {
    if (selectedOrder) {
        ...
        const deadline = selectedOrder.start ? new Date(selectedOrder.start) : new Date(); // Use start as the deadline
...
const handleUpdateOrder = async () => {
  ...
  try {
      const response = await axios.put(
          `${import.meta.env.VITE_BACKEND_URL}/api/workorders/${localSelectedOrder._id}`,
          payload,
          {
              headers: {
                  Authorization: `Bearer ${token}`,
      ...
      setWorkOrders((prevOrders) =>
          prevOrders.map((order) => (order._id === response.data._id ? response.data : order))
      );
...
{headerGroup.headers.map((column) => {
  ...
  return (
    <th {...columnProps} key={columnProps.key}> {/* Use columnProps.key directly */}
      {column.render("Header")}
      <span>
        {column.isSorted ? (column.isSortedDesc ? <FaSortDown /> : <FaSortUp />) : <FaSortDown />}
      </span>
    </th>
  );
})}
  ...
  {page.map((row) => {
    prepareRow(row);
    const { key, ...rowProps } = row.getRowProps(); // Destructure key from rowProps
    return (
      <tr key={key} {...rowProps}> {/* Pass key directly and spread the rest */}
        {row.cells.map((cell) => (
  ...
  console.log("Calendar Deadline:", localSelectedOrder.deadline), // Log the deadline before rendering
  <Calendar
    onChange={setLocalSelectedOrder}
    value={localSelectedOrder.deadline ? new Date(localSelectedOrder.deadline) : new Date()} // Fallback to current date
  />
)}
                ...
                <>
                    ...
                    <div className="mb-3">
                        ...
                        <input
                            type="date"
                            id="deadline"
                            value={localSelectedOrder.deadline ? localSelectedOrder.deadline.toISOString().split("T")[0] : ''}
                            onChange={(e) =>
                                setLocalSelectedOrder({
                                    ...localSelectedOrder,
 ...
 <div className="mb-3">
      ...
      <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)}>
        ...
        <Modal.Body>
          ...
          <div className="mb-3">
            <label htmlFor="newPriority" className="form-label">Priority</label>
            <select
              id="newPriority"
